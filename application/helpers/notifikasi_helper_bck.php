...
function notif_tracking_order($kode_transaksi){
    $ci = & get_instance();
    $id_penjual = $ci->db->query("SELECT * FROM rb_penjualan WHERE kode_transaksi = '".$kode_transaksi."' ")->row_array();
    $rekening =  $ci->db->query("SELECT * FROM rb_rekening_reseller WHERE id_reseller = '".$id_penjual['id_penjual']."'");
