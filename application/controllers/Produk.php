...
class Produk extends CI_Controller {
		...
		if (config('mode')=='marketplace'){
			...
			if (is_numeric($dari)) {
				...
				if (isset($_POST['submit'])){
					...
					$data['record'] = $this->db->query("SELECT * FROM rb_reseller a LEFT JOIN rb_kota b ON a.kota_id=b.kota_id WHERE verifikasi = 'Y' ORDER BY id_reseller DESC LIMIT $dari,$config[per_page]");
				}
	...
	function rajaongkir_get_provinsi(){
		$records = $this->db->query("SELECT * FROM tb_ro_provinces WHERE province_aktif = 1 ORDER BY province_name ASC");
    ...
    function rajaongkir_get_kota(){
		...
		$records = $this->db->query("SELECT * FROM tb_ro_cities where city_id = 44 and  province_id='$id_province'");
