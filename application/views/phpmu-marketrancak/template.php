...
<head>
    ...
    <noscript><img height="1" width="1" style="display:none"
            src="https://www.facebook.com/tr?id=<?= config('facebook_pixel'); ?>&ev=PageView&noscript=1" /></noscript>
        ...
        <header class="header header--1" data-sticky="true">
            ...
            <div class="header__top">
                <div class="ps-container">
                    ...
                    <div class="header__center" style="padding-left: 18px;">
                        <form class="ps-form--quick-search" action="<?php echo base_url() ?>produk" method="GET" style="width: 95%;">
                    ...
                    <div class="header__right">
                        <div class="header__actions">
                            ...
                            if ($this->session->level == 'konsumen') {
                                ...
                                $sopir = $this->db->query("SELECT id_sopir FROM rb_sopir WHERE id_konsumen='" . $this->session->id_konsumen . "'")->row_array();
                                $cek_pesanan_sopir = $this->db->query("SELECT * FROM rb_penjualan a WHERE a.kurir='$sopir[id_sopir]' AND a.proses!='4' AND service='SOPIR'")->num_rows();
                                $total_notif = $komplain_beli->num_rows() + $cek_pesanan_sopir;

                                // Tampilkan hanya icon user jika sudah login
                                echo "
                                    <a class='header__extra' href='" . base_url() . "members/profile' title='Akun Saya'>
                                        <i class='icon-user'></i>
                                        " . ($total_notif > 0 ? "<span class='badge badge-secondary'>$total_notif</span>" : "") . "
                                    </a>
                                    ";
                            } else {
                                // Jika belum login, tampilkan tombol untuk login
                                echo "
                                    <a class='header__extra' href='#' data-toggle='modal' data-target='.bd-example-modal-lg'>
                                        <i class='icon-user'></i>
                                        </a>
                                    ";
                            }
