...
<div class="ps-vendor-dashboard pro" style='margin-top:10px'>
    <div class="container">
      <div class="ps-section__content">
        ...
        ?>
          <table class="table table-condensed iconset">
                    <thead>
                      <tr>
                    ...
                    <?php 
                        echo "<tr>
                                ...
                                <td><select name='aa' class='combobox form-control form-mini required' onchange=\"changeValue(this.value)\" required autofocus>
                              ...
                              $cek_keranjang = $this->db->query("SELECT * FROM rb_penjualan_detail where id_penjualan='".$this->session->idp."'");
                            if ($cek_keranjang->num_rows()<=0){
                              echo "<tr><td colspan='7'><center style='padding:50px 0px'>Keranjang Belanja Masih Kosong...<br>Lihat Produk <a style='font-weight:700; text-decoration:underline' href='".base_url()."produk/perusahaan'>disini.</center></td></tr>";
                            }
                            echo "<tbody>";
                    echo form_close();
                ...
                <?php if ($cek_keranjang->num_rows()>0){ ?>
                <button type='submit' name='selesai' class='ps-btn margin-btn spinnerButton' href=''>Selesai dan Proses</button>
                <a class='ps-btn ps-btn--black ml-3 margin-btn' href='<?php echo base_url(); ?>produk/perusahaan'>Lihat Produk</a>
          <?php } echo form_close(); ?>
          </div>
      </div>
        </div>
