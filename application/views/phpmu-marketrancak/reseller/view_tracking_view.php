        ...
        <div class="ps-section__content">
            <div class="table-responsive">
                ...
                if ($cek_onl->num_rows()>=1 OR $cek_digital->num_rows()>=1){ // Cek Transaksi Online
                        ...
                        if ($cek_pembayaran->num_rows()>='1'){
                            ...
                            if (is_numeric($cek_payment['catatan'])!='1'){
                                ...
                                echo "<table class='table table-sm table-hover'>
                                    ...
                                    <tbody>";
                                        $nooo = 1;
                                        $cektransaksi = $this->mm->get('rb_penjualan',array('where'=>array('kode_transaksi'=>$judul)),'roar');
                                        $getpenjual = $this->mm->get('rb_reseller',array('join'=>array(array('rb_rekening_reseller','rb_rekening_reseller.id_reseller = rb_reseller.id_reseller','left')), 'where'=>array('rb_reseller.id_reseller'=>$cektransaksi['id_penjual'])));
                                        //print_r($getpenjual);die();
                                        //$rekening = $this->model_app->view('rb_rekening');
                                        //foreach ($rekening->result_array() as $row){
                                        foreach($getpenjual as $row){
                                            echo "<tr><td>$nooo</td>
                                                <td>$row[nama_bank]</td>
                    ...
                    echo "<a target='_BLANK' class='ps-btn ps-btn--fullwidth' href='".base_url()."konfirmasi/index'>Konfirmasi Pembayaran </a><br>";   
                    /*
